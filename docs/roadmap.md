...
로드맵은 **중간고사를 기준으로 두 시즌으로 나눠서 구성했어요!**

- **1~5주차 (중간고사 이전):** 백엔드 기반 위에 LLM을 연동하는 방법.
- **6~10주차 (중간고사 이후):** LLM을 더 잘 다루기 위한 여러 전략들.

---

## 전체 흐름 요약

### 1~5주차: 서버 개발과 LLM 연동 (중간고사 이전) ✅ 완료
...
| 5주차 | 한 사용자가 여러 URL을 한꺼번에 처리할 수 있는가? | URL 크롤링, async/await, asyncio.gather | ← 여기까지 완료                  |
...
| 주차   | 주제                                    | 도입                  | 한계점                                    |
| ---- | --------------------------------------- | --------------------- | ---------------------------------------- |
| 6주차  | LLM이 원하는 형태로 출력하게 만들 수 있는가?           | 역할 지정, Few-shot      | 프롬프트만으로 해결이 안 되는 지식 한계가 있습니다           |
| 7주차  | LLM이 모르는 외부 정보를 실시간으로 주입할 수 있는가?      | 임베딩, 벡터 DB           | 특정 도메인에서 일반 모델의 한계가 드러납니다               |
| 8주차  | 특정 도메인에 맞게 모델 자체를 변경할 수 있는가?          | 데이터셋 구성, LoRA        | 모델이 커지면 서빙 비용과 속도 문제가 생깁니다              |
| 9주차  | 모델을 더 작고 빠르게 만들 수 있는가?                | 양자화, 지식 증류           | 지금까지 배운 기법을 통합할 차례입니다                   |
| 10주차 | 지금까지 배운 기법을 하나의 프로젝트로 구현할 수 있는가?      | 심화 프로젝트              | 완성입니다                                  |
