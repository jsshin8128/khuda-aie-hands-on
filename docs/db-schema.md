...
| url            | TEXT    | NONE                                | 요청의 url. 추출 실패 시 NULL.                                                  | https://fastapi.tiangolo.com/tutorial/                                                                              |
| title          | TEXT    | NONE                                | 크롤링 시 페이지에서 추출한 제목. 추출 실패 시 NULL.                                        | FastAPI 완벽 가이드                                                                                                      |
| content_text   | TEXT    | NOT NULL                            | 크롤링한 본문 원문. 필수.                                                         | FastAPI는 Python의 타입 힌트를 활용해 빠르고 안전한 API를 작성할 수 있는 프레임워크입니다.                                                         |
| output_json    | TEXT    | NOT NULL                            | 요약 결과(SummaryResponse) 전체를 JSON 문자열로 저장. 5주차부터 difficulty, read_time, recommended_for 포함. | {"summary":"...","key_points":["..."],"difficulty":"중급","read_time":12,"recommended_for":"...","meta":{...}} |
| prompt_version | TEXT    | NOT NULL                            | 요약에 사용한 프롬프트 버전. 나중에 구분용.                                              | v2.0                                                                                                                |
...
| id  | url                                    | title                    | content_text                      | output_json                          | prompt_version | created_at          |
| --- | -------------------------------------- | ------------------------ | --------------------------------- | ------------------------------------ | -------------- | ------------------- |
| 1   | https://fastapi.tiangolo.com/tutorial/ | FastAPI 완벽 가이드       | FastAPI는 Python의 타입 힌트를...  | {"recommended_for":"...","difficulty":"초급","read_time":8,"summary":"...","key_points":[...],"meta":{...}} | v2.0           | 2025-03-28 10:30:00 |
| 2   | https://blog.example.com/rust-async    | Rust async runtime 변경점 | Rust 1.80에서 async runtime이...  | {"recommended_for":"...","difficulty":"중급","read_time":12,"summary":"...","key_points":[...],"meta":{...}} | v2.0           | 2025-03-28 10:31:00 |
...
`SummaryListItem`에서 `created_at`이 빠지고 `url`이 추가됩니다. 단건 조회에도 `url`이 추가됩니다.
...
```sql
SELECT id, url, title, content_text, output_json, prompt_version, created_at
FROM summaries
WHERE id = :id;
```
...
## 5주차: SQLAlchemy ORM 모델 (async)

5주차에서 `url` 컬럼이 추가되고, DB 접근이 비동기로 전환됩니다.
...
class Summary(Base):
    ...
    id             = Column(Integer, primary_key=True, autoincrement=True)
    url            = Column(Text, nullable=True)    # 크롤링 시 사용자가 보낸 URL
    title          = Column(Text, nullable=True)    # 크롤링 시 페이지에서 추출. 실패 시 None
    content_text   = Column(Text, nullable=False)
    output_json    = Column(Text, nullable=False)   # json.dumps()로 저장
    prompt_version = Column(Text, nullable=False)
    created_at     = Column(Text, server_default=func.now(), nullable=False)
...
DATABASE_URL = "sqlite+aiosqlite:///./summary.db"  # aiosqlite 드라이버 사용
