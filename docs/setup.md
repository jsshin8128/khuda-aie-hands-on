...
명령어를 외우려고 하지 마시고, **왜 이걸 하는지** 이해해보세요! 나중에 다른 에러 상황에서도 나도 "아, 여기서 막혔구나"라고 생각하면서 스스로 문제를 해결할 수 있습니다.
...
트랙장과 똑같은 상태로 맞추고 싶다면 아래 두 줄만 실행하세요.
...
**왜 이렇게 하냐면,** `git fetch upstream`은 upstream의 최신 내용만 가져오고 로컬 파일은 건드리지 않습니다. 그다음 `git reset --hard upstream/main`으로 로컬 `main`을 upstream과 똑같이 덮어씁니다. 단, 로컬에서 수정·커밋한 내용은 모두 사라지므로, 주의하세요. 실습을 원활히 진행하기 위해서 해당 명령어를 사용하겠습니다.
...
3주차부터 SQLite DB(`summary.db`)를 사용합니다. 터미널에서 직접 DB를 조회하거나 수정할 때는 **sqlite3** CLI로 실행합니다. 미리 설치해 두세요.
